 ...
 *     Version        0.57       Add API to support WFOV mode, including get WFOV mode, get sensor mount type
...
typedef enum {
    ...
    /**
     * CAMERA_STREAM_CONFIGURATION_MODE_STILL_DURING_VIDEO is used to select PSYS pipeline,
     * Create only still pipe
     * TuningMode and MediaCtlConfig still pipe.
     */
    CAMERA_STREAM_CONFIGURATION_MODE_STILL_DURING_VIDEO,

    CAMERA_STREAM_CONFIGURATION_MODE_END
} camera_stream_configuration_mode_t;

/** The CAMERA3_* enums are replaced with CAMERA_* enums, Please do NOT use them any more. */
typedef enum {
    CAMERA3_STREAM_OUTPUT = 0,        /**deprecated */
    CAMERA3_STREAM_INPUT = 1,         /**deprecated */
    CAMERA3_STREAM_BIDIRECTIONAL = 2, /**deprecated */
    CAMERA3_NUM_STREAM_TYPES          /**deprecated */
} camera3_stream_type_t;

typedef enum {
    CAMERA3_STREAM_CONFIGURATION_NORMAL_MODE = 0,                   /**deprecated */
    CAMERA3_STREAM_CONFIGURATION_CONSTRAINED_HIGH_SPEED_MODE = 1,   /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_START = 0x8000,        /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_AUTO,                  /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_HDR,                   /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_ULL,                   /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_HLC,                   /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_CUSTOM_AIC,            /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_VIDEO_LL,              /**deprecated */
    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_STILL_CAPTURE,         /**deprecated */

    CAMERA3_VENDOR_STREAM_CONFIGURATION_MODE_END
...
/**
 * \enum camera_view_projection_type_t: camera view projection type.
 */
typedef enum {
...
} camera_view_projection_type_t;

/**
 * \enum camera_view_rotation_t: Used to control camera view rotation.
 */
typedef struct
{
...
} camera_view_rotation_t;

/**
 * \enum camera_view_fine_adjustments_t: Used to fine control camera view window.
 */
typedef struct
{
...
} camera_view_fine_adjustments_t;

/*!< camera_view_projection_t: Set view projection*/
typedef struct
{
    camera_view_projection_type_t type;   /*!< type: The projection type desired*/
    float cone_angle;                /*!< cone_angle used only for ia_view_projection_conical*/
} camera_view_projection_t;
...
public:
     ...
     *
     * \param[in] ctrlId: Indicates which ISP feature will be controled.
    ...
    /**
     * \brief Set LTM live tuning data
     *
     * \param[in] data: The LTM tuning data used to control LTM algo.
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    int setLtmTuningData(const void* data);

    /**
     * \brief Get LTM live tuning data
     *
     * \param[out] data: The LTM tuning data used to control LTM algo.
     *
     * \return 0 if find the corresponding data, otherwise non-0 value is returned.
     */
    int getLtmTuningData(void* data) const;
     ...
     *       Without calling this funtion, the feature will be ignored by HAL.
     *       User no need to call this funtion if he have no new ISP control feature to enable.
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    ...
    /**
     * \brief Get WFOV mode enabled or not
     *
     * \param[out] wfov  WFOV mode enabled or not
     *
     * \return 0 if wfov was set, otherwise non-0 value is returned.
     */
    int getWFOV(uint8_t& wfov) const;
    /**
    ...
    /**
     * \brief Set view projection
     *
     * \param[in] viewProjection  view projection and cone angle if projection type is cone
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    int setViewProjection(camera_view_projection_t viewProjection);
    /**
     * \brief Get view projection
     *
     * \param[out] viewProjection  view projection and cone angle
     *
     * \return 0 if viewProjection was set, otherwise non-0 value is returned.
     */
    int getViewProjection(camera_view_projection_t& viewProjection) const;

    /**
     * \brief Set view fine adjustments
     *
     * \param[in] viewFineAdj the view fine adjustments including horizontal/vertical shift and window rotation
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    int setViewFineAdjustments(camera_view_fine_adjustments_t viewFineAdj);
    /**
     * \brief Get view fine adjustments
     *
     * \param[out] viewFineAdj the view fine adjustments including horizontal/vertical shift and window rotation
     *
     * \return 0 if viewFineAdj was set, otherwise non-0 value is returned.
     */
    int getViewFineAdjustments(camera_view_fine_adjustments_t& viewFineAdj) const;

    /**
     * \brief Set view rotation
     *
     * \param[in] viewRotation  the view rotation angles
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    int setViewRotation(camera_view_rotation_t viewRotation);
    /**
     * \brief Get view rotation
     *
     * \param[out] viewRotation  the view rotation angles
     *
     * \return 0 if viewRotation was set, otherwise non-0 value is returned.
     */
    int getViewRotation(camera_view_rotation_t& viewRotation) const;

    /**
     * \brief Set camera device rotation
     *
     * \param[in] cameraRotation  the camera device rotation angles
     *
     * \return 0 if set successfully, otherwise non-0 value is returned.
     */
    int setCameraRotation(camera_view_rotation_t cameraRotation);
    /**
     * \brief Get camera device rotation
     *
     * \param[out] cameraRotation  the camera device rotation angles
     *
     * \return 0 if viewRotation was set, otherwise non-0 value is returned.
     */
    int getCameraRotation(camera_view_rotation_t& cameraRotation) const;

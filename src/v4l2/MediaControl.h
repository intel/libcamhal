...
struct McFormat {
    ...
    McFormat() { entity = 0; pad = 0; stream = 0; formatType = 0; selCmd = 0;
        top = 0; left = 0; width = 0; height = 0;
        type = RESOLUTION_MAX; pixelCode = 0; }
};
...
struct MediaCtlConf {
    vector <McCtl> ctls;
    vector <McLink> links;
    vector <McRoute> routes;
    vector <McFormat> formats;
    vector <McOutput> outputs;
    vector <McVideoNode> videoNodes;
    int mcId;
    int outputWidth;
    int outputHeight;
    vector <ConfigMode> configMode;
    int format;
     ...
     */
    MediaCtlConf() { mcId = -1; outputWidth = 0; outputHeight = 0; format = -1; vbp = -1;}
};
...
private:
    ...
    int setMediaMcCtl(int cameraId, vector <McCtl> ctls);
    int setMediaMcLink(vector <McLink> links);

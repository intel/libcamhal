...
int BufferQueue::onFrameAvailable(Port port, const shared_ptr<CameraBuffer> &camBuffer)
{
...
int BufferQueue::qbuf(Port port, const shared_ptr<CameraBuffer> &camBuffer)
{
    LOG2("%s CameraBuffer %p for port:%d", __func__, camBuffer.get(), port);
...
int BufferQueue::waitFreeBuffersInQueue(ConditionLock& lock,
                                        std::map<Port, shared_ptr<CameraBuffer> > &cInBuffer,
                                        std::map<Port, shared_ptr<CameraBuffer> > &cOutBuffer)
{
    ...
    for (auto& input: mInputQueue) {
        ...
        while (inputQueue.empty()) {
            ...
            ret = mFrameAvailableSignal.waitRelative(lock, kWaitDuration);
    ...
    for (auto& output: mOutputQueue) {
        ...
        while (outputQueue.empty()) {
            ...
            ret = mOutputAvailableSignal.waitRelative(lock, kWaitDuration);
...
int BufferQueue::allocProducerBuffers(int camId, int bufNum)
{
    ...
    Check(!mBufferProducer, BAD_VALUE ,"@%s: Buffer Producer is nullptr", __func__);
    ...
    for (const auto& item : mInputFrameInfo) {
        ...
        LOG1("%s fmt:%s (%dx%d)", __func__, CameraUtils::format2string(srcFmt), srcWidth, srcHeight);

        int32_t size = CameraUtils::getFrameSize(srcFmt, srcWidth, srcHeight);
        int memType = mBufferProducer->getMemoryType();
#ifdef ENABLE_VIRTUAL_IPU_PIPE
        size += ATEUnit::getATEPayloadSize();
        LOG2("ATEPayload Size %d, total size %d", ATEUnit::getATEPayloadSize(), size);
#endif
        ...
        for (int i = 0; i < bufNum; i++) {
            ...
            case V4L2_MEMORY_USERPTR:
                ...
                Check(!camBuffer, NO_MEMORY, "Allocate producer userptr buffer failed");
                break;

            case V4L2_MEMORY_MMAP:
                camBuffer = make_shared<CameraBuffer>(camId, BUFFER_USAGE_PSYS_INPUT,
                                                      V4L2_MEMORY_MMAP, size, i, srcFmt);
                Check(!camBuffer, NO_MEMORY, "Allocate producer mmap buffer failed");

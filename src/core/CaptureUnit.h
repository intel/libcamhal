...
/**
  ...
  */
class CaptureUnit : public BufferProducer, public DeviceCallback {
...
public:
     ...
     * 3. Queue Isa state and config buffer if Isa is enabled
     *
     ...
     */
    virtual int allocateMemory(Port port, const shared_ptr<CameraBuffer> &camBuffer);
     ...
     * 3. Isa adaptor deinit
     * 4. Destory the poll thread
     */
    virtual void deinit();
     ...
     *
     * 1. Stream on
     * 2. Queue Isa buffer queue and Isa Stream on
     * 3. Running the pool Thread
     */
    virtual int start();
     ...
     *
     * 1. Stream off
     * 2. Isa Capture stream off is enable Isa
     * 3. Release all the buffer queue
     * 4. Stop the pool thread.
     */
    virtual int stop();
     ...
     * \return OK if succeed, other value indicates failed
     */
    virtual int configure(const map<Port, stream_t>& outputFrames, const vector<ConfigMode>& configModes);
...
private:
    ...
    Port findDefaultPort(const map<Port, stream_t>& frames) const;
    ...
    bool isNewConfiguration(const map<Port, stream_t>& outputFrames);
...
private:
    ...
    map<Port, stream_t> mOutputFrameInfo;
    vector<DeviceBase*> mDevices;

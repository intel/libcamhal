...
#define LOG_TAG "Hp4KPostGDC"

#include "Hp4KPostGDC.h"
...
Hp4KPostGDC::Hp4KPostGDC() : PGBase(PG_ID)
{
    for (int i = 0; i < (VPOSTGDC_HQ_4K_TERMINAL_ID_N); i++) {
        mHp4KPostGDCFrameFmtTypeList[i] = IA_CSS_N_FRAME_FORMAT_TYPES;
    }
    mFrameFormatType = mHp4KPostGDCFrameFmtTypeList;
...
int Hp4KPostGDC::configTerminal()
{
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_GET] = PGBase::getCssFmt(mSrcFrame[MAIN_PORT].mFormat);
    if (mDstFrame.find(SECOND_PORT) != mDstFrame.end())
        mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_DISPLAY] = PGBase::getCssFmt(mDstFrame[SECOND_PORT].mFormat);
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_MAIN] = PGBase::getCssFmt(mDstFrame[MAIN_PORT].mFormat);
    if (mDstFrame.find(THIRD_PORT) != mDstFrame.end())
        mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_PP] = PGBase::getCssFmt(mDstFrame[THIRD_PORT].mFormat);
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_TERMINAL] = PGBase::getCssFmt(mSrcFrame[MAIN_PORT].mFormat);
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL] = IA_CSS_DATA_FORMAT_Y800;
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_TERMINAL] = PGBase::getCssFmt(mSrcFrame[MAIN_PORT].mFormat);
    mHp4KPostGDCFrameFmtTypeList[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL] = IA_CSS_DATA_FORMAT_Y800;
    ...
    /* The second port is disable, add the display output terminal to mDisableDataTermials. */
     if (mDstFrame.find(SECOND_PORT) == mDstFrame.end())
        mDisableDataTermials.push_back(VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_DISPLAY);

    /* The third port is disable, add the pp output terminal to mDisableDataTermials. */
    if (mDstFrame.find(THIRD_PORT) == mDstFrame.end())
        mDisableDataTermials.push_back(VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_PP);

    /* Add the dummy terminal to mDisableDataTermials. */
    mDisableDataTermials.push_back(VPOSTGDC_HQ_4K_TERMINAL_ID_GET_NS_TERMINAL);

    return OK;
}

int Hp4KPostGDC::setTerminalParams(const ia_css_frame_format_type* frameFormatTypes)
{
    ...
    for (int i = 0; i < mTerminalCount; i++) {
        ...
        if (i == VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_MAIN) {
            config = mDstFrame[MAIN_PORT];
        }

        if (mDstFrame.find(SECOND_PORT) != mDstFrame.end() && i == VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_DISPLAY) {
            config = mDstFrame[SECOND_PORT];
        }

        if (mDstFrame.find(THIRD_PORT) != mDstFrame.end() && i == VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_PP) {
            config = mDstFrame[THIRD_PORT];
        }

        terminalParam->frame_format_type = frameFormatTypes[i];
        terminalParam->dimensions[IA_CSS_COL_DIMENSION] = ((i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL ||
                                                            i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL) ?
                                                            config.mWidth / 8 : config.mWidth);
        terminalParam->dimensions[IA_CSS_ROW_DIMENSION] = ((i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL ||
                                                            i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL) ?
        ...
        terminalParam->bpp = (i == VPOSTGDC_HQ_4K_TERMINAL_ID_GET ? 16 : 8);
        terminalParam->bpe = terminalParam->bpp;
        terminalParam->stride = ((i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL ||
                                  i == VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL) ?
                                  terminalParam->dimensions[IA_CSS_COL_DIMENSION] * 2 :
                                  i == VPOSTGDC_HQ_4K_TERMINAL_ID_GET ? config.mStride * 2 : config.mStride);
...
int Hp4KPostGDC::prepareTerminalBuffers(CameraBufferPortMap &inBuf, CameraBufferPortMap &outBuf)
{
    int ret = OK;
    ...
    mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_GET] = ciprBuf;
    ...
    if (!mAllocTNRBuffers) {
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_TERMINAL] = ciprBuf;
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL] = ciprBuf;
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_TERMINAL] = ciprBuf;
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL] = ciprBuf;

        mAllocTNRBuffers = true;
    } else {
        LOG1("%s: swith the TNR input and output buffers", __func__);
        tmpBuffer = mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_TERMINAL];
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_TERMINAL] =
            mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_TERMINAL];
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_TERMINAL] = tmpBuffer;

        tmpBuffer = mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL];
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_GET_RSIM_TERMINAL] =
            mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL];
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_TNR5_2_2_PUT_RSIM_TERMINAL] = tmpBuffer;
    }
    ...
    mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_MAIN] = ciprBuf;

    LOG1("liangc1 2 %p, %d, 3 %p, %d", outPtr[SECOND_PORT], outFd[SECOND_PORT], outPtr[THIRD_PORT], outFd[THIRD_PORT]);
    if (outPtr[SECOND_PORT] || (outFd[SECOND_PORT] >= 0)) {
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_DISPLAY] = ciprBuf;
    }
    ...
    if (outPtr[THIRD_PORT] || (outFd[THIRD_PORT] >= 0)) {
        ...
        mTerminalBuffers[VPOSTGDC_HQ_4K_TERMINAL_ID_PUT_PP] = ciprBuf;
    }

    return ret;
}

int Hp4KPostGDC::iterate(CameraBufferPortMap &inBuf, CameraBufferPortMap &outBuf,

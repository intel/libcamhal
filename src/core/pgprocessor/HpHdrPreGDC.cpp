...
#define LOG_TAG "HpHdrPreGDC"

#include "HpHdrPreGDC.h"
...
HpHdrPreGDC::HpHdrPreGDC() : PGBase(PG_ID)
{
    for (int i = 0; i < VPREGDC_HP_TERMINAL_ID_N; i++) {
        mHpHdrPreGDCFrameFmtTypeList[i] = IA_CSS_N_FRAME_FORMAT_TYPES;
    }
    mFrameFormatType = mHpHdrPreGDCFrameFmtTypeList;
    CLEAR(mParamPayload);
}

HpHdrPreGDC::~HpHdrPreGDC()
{
...
int HpHdrPreGDC::configTerminal()
{
    mHpHdrPreGDCFrameFmtTypeList[VPREGDC_HP_TERMINAL_ID_GET] = PGBase::getCssFmt(mSrcFrame[MAIN_PORT].mFormat);
    mHpHdrPreGDCFrameFmtTypeList[VPREGDC_HP_TERMINAL_ID_PUT] = PGBase::getCssFmt(mDstFrame[MAIN_PORT].mFormat);

    return OK;
}

int HpHdrPreGDC::setTerminalParams(const ia_css_frame_format_type* frameFormatTypes)
{
...
int HpHdrPreGDC::prepareTerminalBuffers(CameraBufferPortMap &inBuf, CameraBufferPortMap &outBuf)
{
    int ret = OK;
    ...
    mTerminalBuffers[VPREGDC_HP_TERMINAL_ID_GET] = ciprBuf;
    ...
    mTerminalBuffers[VPREGDC_HP_TERMINAL_ID_PUT] = ciprBuf;

    return ret;
}


int HpHdrPreGDC::iterate(CameraBufferPortMap &inBuf, CameraBufferPortMap &outBuf,
...
int HpHdrPreGDC::decodeStats(ia_binary_data *statistics)
{
    int ret = OK;

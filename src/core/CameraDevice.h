...
public:
     ...
     *
     * 1.Related classes init: CaptureUnit, SofSource, CsiMeta, 3AControl, lensCtrl
...
private:
    ...
    shared_ptr<CameraBuffer> userBufferToCameraBuffer(Port port, camera_buffer_t *ubuffer);

    CaptureUnit* createBufferProducer();
    map<Port, stream_t> selectProducerConfig(const stream_config_t *streamList, int mcId);
...
private:
    ...
    map<int, Port> mStreamIdToPortMap;
    vector<int> mSortedStreamIds; // Used to save sorted stream ids with descending order.
    CaptureUnit *mProducer;

    ProcessorManager* mProcessorManager;
    vector<BufferQueue*> mProcessors;

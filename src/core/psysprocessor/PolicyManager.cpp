...
int PolicyManager::wait(string executorName)
{
    ...
    if (bundle->mWaitingCount < bundle->mExecutorNum) {
        ...
        const int64_t kWaitDuration = 2000000000; // 2 seconds
        int ret = bundle->mCondition.waitRelative(lock, kWaitDuration);
        Check(ret == TIMED_OUT, ret, "%s: wait for other executors timeout", executorName.c_str());
    } else {
        bundle->mWaitingCount = 0;
        bundle->mCondition.broadcast();
    }

...
{
    Check(!inBuf || !outBuf, -1, "@%s,inBuf:%p, outBuf:%p", __func__, inBuf, outBuf);
    Check(!CameraUtils::isRaw(srcFmt), -1, "[%s] Unsupported input format, srcFmt:%s",
          __func__, CameraUtils::format2string(srcFmt));

    LOG2("@%s, inLen:%d, outLen:%d, width:%d, height:%d, srcFmt:%s, dstFmt:%s, shift:%d",
            __func__, inLen, outLen, width, height,
            CameraUtils::format2string(srcFmt), CameraUtils::format2string(dstFmt), shift);
    ...
    Check(inLen < line_size * height, -1, "[%s] No enough vector data, inLen:%d %d", __func__, inLen, line_size * height);
    Check(outLen < dstStride * height, -1,
...
{
    Check((inBuf == nullptr || outBuf == nullptr), BAD_VALUE, "Invalid input(%p) or output buffer(%p)", inBuf, outBuf);

    unsigned int x, y;
    unsigned short bayer_data[4];

    LOG2("%s srcFmt %s => dstFmt %s %dx%d", __func__,
        CameraUtils::format2string(srcFmt), CameraUtils::format2string(dstFmt), width, height);
    ...
    if (CameraUtils::isVectorRaw(srcFmt)) {
        ...
        switch(srcFmt) {
            ...
            case V4L2_PIX_FMT_SGRBG8V32:
                ...
                break;
            default:
                LOGW("%s: srcFmt %s is not supported", __func__, CameraUtils::format2string(srcFmt));
                return BAD_VALUE;
        }

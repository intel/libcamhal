...
void CameraDump::writeData(const void* data, int size, const char* fileName) {
    Check((data == nullptr || size == 0 || fileName == nullptr), VOID_VALUE, "Nothing needs to be dumped");

    FILE *fp = fopen (fileName, "w+");
    Check(fp == nullptr, VOID_VALUE, "open dump file %s failed", fileName);
...
static string getNamePrefix(int cameraId, ModuleType_t type, Port port)
{
    ...
    if (icamera::CameraDump::isDumpFormatEnable(DUMP_FORMAT_IQSTUDIO)) {
        snprintf(prefix, (MAX_NAME_LEN - 1), "%s/name#%s", dumpPath, sensorName);
    } else {
        if (port == INVALID_PORT) {
            snprintf(prefix, (MAX_NAME_LEN - 1), "%s/cam%d_%s_%s", dumpPath, cameraId,
                 sensorName, ModuleName[type]);
        } else {
            snprintf(prefix, (MAX_NAME_LEN - 1), "%s/cam%d_%s_%s_port%d", dumpPath, cameraId,
                 sensorName, ModuleName[type], port);
        }
    }
...
static string getAiqSettingAppendix(int cameraId, long sequence)
{
    ...
    if (aiqResults == nullptr) {
        ...
        Check((aiqResults == nullptr), string(settingAppendix), "Cannot find available aiq result.");
    }
    ...
    Check((sensorExposure == nullptr || exposure == nullptr), string(settingAppendix), "Cannot find aiq exposures");
    ...
    if (icamera::CameraDump::isDumpFormatEnable(DUMP_FORMAT_IQSTUDIO)) {
        ...
        if (strstr(sensorName, "imx185") != nullptr) {
            ...
            dg = nSteps * AiqUtils::getSensorDigitalGain(cameraId, exposure->digital_gain);
            ispDg = nSteps * AiqUtils::getIspDigitalGain(cameraId, exposure->digital_gain);
            LOG2("%s: converted AG: %f, DG: %f ispDG: %f for %s", __func__, ag, dg, ispDg, sensorName);
        }
        ...
        if (PlatformData::isUsingIspDigitalGain(cameraId)) {
            dg = AiqUtils::getSensorDigitalGain(cameraId, exposure->digital_gain);
            ispDg = AiqUtils::getIspDigitalGain(cameraId, exposure->digital_gain);
        }
...
static string formatBinFileName(int cameraId, const char *prefix, BinParam_t *binParam)
{
    ...
    case BIN_TYPE_SENSOR_METADATA:
        snprintf(fileName, (MAX_NAME_LEN - 1),
                 ...
                 CameraUtils::format2string(binParam->mParam.metaFormat));
        break;
    case BIN_TYPE_BUFFER:
        ...
        return formatFrameFileName(prefix, appendix.c_str(),
                                   CameraUtils::format2string(binParam->bParam.format),
                                   binParam->sequence,
...
{
    Check(camBuffer == nullptr, VOID_VALUE, "invalid param");
    ...
    string prefix   = getNamePrefix(cameraId, type, port);
    ...
    string fileName = formatFrameFileName(prefix.c_str(), appendix.c_str(),
                                          CameraUtils::format2string(camBuffer->getFormat()),
...
{
    Check(binParam == nullptr, VOID_VALUE, "invalid param");
    ...
    string prefix   = getNamePrefix(cameraId, binParam->mType, INVALID_PORT);

...
int SWJpegEncoder::encode(const InputBuffer &in, const OutputBuffer &out)
{
    ...
    LOG2("@%s:\n\t IN  = {buf:%p, w:%u, h:%u, sz:%u, stride:%d, f:%s}" \
            ...
            __FUNCTION__,
            in.buf, in.width, in.height, in.size, in.stride, icamera::CameraUtils::format2string(in.fourcc),
    ...
    if (useMultiThreadEncoding(in.width, in.height))
        status = swEncodeMultiThread(in, out);
    else
        status = swEncode(in, out);

    if (status < 0)
        goto exit;

    LOG2("@%s encode, total consume:%ums", __FUNCTION__, (unsigned)((CameraUtils::systemTime() - startTime) / 1000000));
    return mJpegSize;
exit:
    return (mJpegSize = -1);
}
...
{
    LOG2("@%s, line:%d, width:%d, height:%d", __FUNCTION__, __LINE__, width, height);
    bool ret = false;
    ...
    LOG2("@%s, line:%d, ret:%d", __FUNCTION__, __LINE__, ret);
    return ret;
}
...
{
    LOG2("@%s, line:%d, use the libjpeg to do sw jpeg encoding", __FUNCTION__, __LINE__);
    int status = 0;
    Codec encoder;
...
void SWJpegEncoder::init(unsigned int threadNum)
{
    ...
    LOG2("@%s, line:%d, thread number, pass:%d, real:%d", __FUNCTION__, __LINE__, threadNum, num);
...
void SWJpegEncoder::config(const InputBuffer &in, const OutputBuffer &out)
{
    ...
    for (unsigned int i = 0; i < mSwJpegEncoder.size(); i++) {
        ...
        */
        cfg.height = ALIGN_16(in.height / mSwJpegEncoder.size());
        ...
        cfg.outBufSize = (out.size - DEST_BUF_OFFSET) / in.height * cfg.height;
        ...
        LOG2("@%s, line:%d, the %d picture thread cfg", __FUNCTION__, __LINE__, i);
        LOG2("@%s, line:%d, cfg.width:%d, cfg.height:%d", __FUNCTION__, __LINE__, cfg.width, cfg.height);
        LOG2("@%s, line:%d, cfg.fourcc:%d, cfg.quality:%d", __FUNCTION__, __LINE__, cfg.fourcc, cfg.quality);
        LOG2("@%s, line:%d, cfg.inBufY:%p, cfg.inBufUV:%p", __FUNCTION__, __LINE__, cfg.inBufY, cfg.inBufUV);
        LOG2("@%s, line:%d, cfg.outBuf:%p, cfg.outBufSize:%d", __FUNCTION__, __LINE__, cfg.outBuf, cfg.outBufSize);
    }
}
...
int SWJpegEncoder::doJpegEncodingMultiThread(void)
{
    ...
    for (unsigned int i = 0; i <mSwJpegEncoder.size(); i++) {
        ...
        LOG2("@%s, new sw jpeg thread name:%s", __FUNCTION__, threadName.c_str());
        encThread = mSwJpegEncoder[i];
        status = encThread->runThread(threadName.c_str());
        if (status != OK) {
            ALOGE("@%s, line:%d, start jpeg thread fail, thread name:%s", __FUNCTION__, __LINE__, threadName.c_str());
            return status;
        }
    }
    ...
    for (unsigned int i = 0; i < mSwJpegEncoder.size(); i++) {
        LOG2("@%s, the %d sw jpeg encoder thread before join!", __FUNCTION__, i);
        encThread = mSwJpegEncoder[i];
...
#define NV12_MCU_SIZE 16
    ...
    for (unsigned int i = 0; i <mSwJpegEncoder.size(); i++) {
        ...
        LOG2("@%s, wr %d segments, size:%d, consume:%ums", __FUNCTION__, i,
             (encThread->getJpegDataSize()- HEADER_TOTAL_LEN - HEADER_EOI_LEN),
...
{
    LOG2("@%s, line:%d, in CodecWorkerThread", __FUNCTION__, __LINE__);
    nsecs_t startTime = CameraUtils::systemTime();
    int ret = swEncode();
    LOG2("@%s one swEncode done!, consume:%ums, ret:%d", __FUNCTION__,
...
{
    LOG2("@%s, quality:%d", __FUNCTION__, quality);
...
doJpegEncoding(const void *y_buf, const void *uv_buf, int fourcc)
{
    ...
    case V4L2_PIX_FMT_NV21:
        NV21ToP411Separate(width, height, mStride, srcY, srcUV, p411);
        break;
    default:
...
void SWJpegEncoder::Codec::termDestination(j_compress_ptr cInfo)
{
    ...
    LOG2("@%s, line:%d, codedSize:%d", __FUNCTION__, __LINE__, dest->codedSize);
}

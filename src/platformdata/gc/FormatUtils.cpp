...
bool isPlanarFormat(int32_t format)
{
    return (format == V4L2_PIX_FMT_NV12 || format == V4L2_PIX_FMT_NV21
         ...
         || format == V4L2_PIX_FMT_YUV422P || format == V4L2_PIX_FMT_P010_BE
...
int32_t getBpl(int32_t format, int32_t width)
{
    ...
    switch (format) {
        ...
        default:
            bpl = width;
            LOGW("bpl defaulting to width for format:%s", CameraUtils::format2string(format));
            break;
    }
    return bpl;
}
...
{
    char buff[32];
    snprintf(buff, sizeof(buff), "%d", bpp);
    std::string bppAsStr = buff;

...
{
public:
    explicit CpfStore(string sensorName) { }
    virtual ~CpfStore() { }
};
#endif
    ...
    public:
        ...
        public:
            CameraInfo() :
                ...
                mISysRawFormat(V4L2_PIX_FMT_SRGGB12V32),
                // LITE_PROCESSING_S
                mKeepFpsDuringDeinterlace(true),
                // LITE_PROCESSING_E
                mUseCrlModule(true),
                mFacing(FACING_BACK),
            ...
            {
                CLEAR(mIsaScaleRawConfig);
            }

            vector <MediaCtlConf> mMediaCtlConfs;
            ...
            int mLensCloseCode;
            bool mEnableAIQ;
            vector<int> mDolVbpOffset;
            vector <OBSetting> mSensorOBSettings;
            ...
            string mCustomAicLibraryName;
            string mCustom3ALibraryName;
            vector <camera_resolution_t> mSupportedISysSizes; // ascending order request
            vector <int> mSupportedISysFormat;
            int mISysFourcc; // the isys output format
            int mISysRawFormat; // the isys raw format if scale enabled
            stream_t mIsaScaleRawConfig; // The config for ISA raw if scale enabled

            vector <int> mPSysFormat; // the psys output format
            vector <TuningConfig> mSupportedTuningConfig;
            vector <LardTagConfig> mLardTagsConfig;
            vector <ConfigMode> mConfigModesForAuto;

            // LITE_PROCESSING_S
            bool mKeepFpsDuringDeinterlace;
            // LITE_PROCESSING_E
            bool mUseCrlModule;
            int mFacing;
            int mOrientation;
            ...
            map<int, supported_stream_config_array_t> mStreamToMcMap;
            Parameters mCapability;

            // CUSTOM_WEIGHT_GRID_S
            vector <WeightGridTable> mWGTable;
            // CUSTOM_WEIGHT_GRID_E
            string mGraphSettingsFile;
            GraphConfigNodes *mGCMNodes;
            vector <MultiExpRange> mMultiExpRanges;
            vector <uint32_t> mSupportedIspControlFeatures;
            int mDVSType;
            ...
            map<int, int> mConfigModeToStreamId;
        };

        vector<CameraInfo> mCameras;
        vector<PolicyConfig> mPolicyConfig;
        CommonConfig mCommonConfig;
    };
private:
    StaticCfg mStaticCfg;

    CpfStore* mKnownCPFConfigurations[MAX_CAMERA_NUMBER];
private:
    /**
    ...
    /**
     * Deinit and clear CpfStore in mKnownCPFConfigurations[]
     *
     */
    void deinitCpfStore();
...
public:
    ...
    /**
     * Check if sensor optical black data is used or not
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \param configMode: config mode
     * \param obSetting: sensor OB setting (output)
     * \return if sensor optical black data is used or not.
     */
    static bool getSensorOBSetting(int cameraId, ConfigMode configMode,
                                   OBSetting& obSetting);
     ...
     */
    static void getDolVbpOffset(int cameraId, vector<int>& dolVbpOffset);
     ...
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \param hdrEnabled: true or false
     * \return the value of exposure number according to different cases: DOL, HDR
     */
    static int getExposureNum(int cameraId, bool hdrEnabled);

    /**
     * Get HDR exposure type
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the value of HDR exposure type
     */
    static int getHDRExposureType(int cameraId);
     ...
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the value of HDR gain type
     */
    static int getHDRGainType(int cameraId);
    ...
    // CUSTOMIZED_3A_S
    /**
     * Get customized Aic library name
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the name of customized Aic library
     */
    static const char* getCustomizedAicLibraryName(int cameraId);

    /**
     * Get customized 3A library name
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the name of customized 3A library
     */
    static const char* getCustomized3ALibraryName(int cameraId);
    // CUSTOMIZED_3A_E
     ...
     */
    static WeightGridTable* getWeightGrild(int cameraId, unsigned short width, unsigned short height, int index);
    // CUSTOM_WEIGHT_GRID_E
     ...
     */
    static void getSupportedISysSizes(int cameraId, vector <camera_resolution_t>& resolutions);
     ...
     */
    static bool getSupportedISysFormats(int cameraId, vector <int>& formats);
    ...
    /**
     * if ISYS scale enabled
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return true if isys scale enabled, false if disabled
     */
    static bool isISysScaleEnabled(int cameraId);
    ...
    /**
     * Get the config for the ISYS RAW output
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the RAW config if ISYS scale enabled.
     */
    static stream_t getIsaScaleRawConfig(int cameraId);
     ...
     * \return the status
     */
    static int getFormatByDevName(int cameraId, const string& devName, McFormat& format);
     ...
     * \return the status
     */
    static int getVideoNodeNameByType(int cameraId, VideoNodeType videoNodeType, string& videoNodeName);
     ...
     * \return the status
     */
    static int getDevNameByType(int cameraId, VideoNodeType videoNodeType, string& devName);

    static bool isIsaEnabled(int cameraId);
     ...
     */
    static void getSupportedTuningConfig(int cameraId, vector <TuningConfig> &configs);
     ...
     */
    static int getConfigModesByOperationMode(int cameraId, uint32_t operationMode, vector <ConfigMode> &configModes);
    ...
    /**
     * Create CpfStore object, initialize mKnownCPFConfigurations[cameraId]
    ...
    /**
     * to get the GraphConfigNodes of CameraInfo
     *
     * \param cameraId: 0 ~ MAX_CAMERA_NUMBER
     * \return the GraphConfigNodes if use graphConfig file.
     */
    static GraphConfigNodes* getGraphConfigNodes(int cameraId);
    ...
    // LITE_PROCESSING_S
    static bool needKeepFpsDuringDeinterlace(int cameraId);
    // LITE_PROCESSING_E
     ...
     */
    static vector<MultiExpRange> getMultiExpRanges(int cameraId);

    static vector<uint32_t> getSupportedIspControlFeatures(int cameraId);

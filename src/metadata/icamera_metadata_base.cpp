...
const size_t icamera_metadata_type_size[ICAMERA_NUM_TYPES] = {
    [ICAMERA_TYPE_BYTE]     = sizeof(uint8_t),
    [ICAMERA_TYPE_INT32]    = sizeof(int32_t),
    [ICAMERA_TYPE_FLOAT]    = sizeof(float),
    [ICAMERA_TYPE_INT64]    = sizeof(int64_t),
    [ICAMERA_TYPE_DOUBLE]   = sizeof(double),
    [ICAMERA_TYPE_RATIONAL] = sizeof(icamera_metadata_rational_t)
};

const char *icamera_metadata_type_names[ICAMERA_NUM_TYPES] = {
    [ICAMERA_TYPE_BYTE]     = "byte",
    [ICAMERA_TYPE_INT32]    = "int32",
    [ICAMERA_TYPE_FLOAT]    = "float",
    [ICAMERA_TYPE_INT64]    = "int64",
    [ICAMERA_TYPE_DOUBLE]   = "double",
    [ICAMERA_TYPE_RATIONAL] = "rational"
};
...
int append_icamera_metadata(icamera_metadata_t *dst,
    ...
    if (dst->entry_capacity - dst->entry_count < src->entry_count) {
        ALOGE("%s: Dst available buffer size for entry is smaller than src needed.", __FUNCTION__);
    }
    ...
    if (dst->data_capacity - dst->data_count < src->data_count) {
        ALOGE("%s: Dst available buffer size for data is smaller than src needed.", __FUNCTION__);
    }
...
static int add_camera_metadata_entry_raw(icamera_metadata_t *dst,
    ...
    if (type >= ICAMERA_NUM_TYPES) {
        ALOGE("%s: Bad type %d", __FUNCTION__, type);
        return ERROR;
    }
...
static void print_data(int fd, const uint8_t *data_ptr, uint32_t tag,
    ...
    static int values_per_line[ICAMERA_NUM_TYPES] = {
        [ICAMERA_TYPE_BYTE]     = 16,
        [ICAMERA_TYPE_INT32]    = 4,
        [ICAMERA_TYPE_FLOAT]    = 8,
        [ICAMERA_TYPE_INT64]    = 2,
        [ICAMERA_TYPE_DOUBLE]   = 4,
        [ICAMERA_TYPE_RATIONAL] = 2,
    };

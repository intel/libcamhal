...
CameraMetadata &CameraMetadata::operator=(const icamera_metadata_t *buffer) {
    if (mLocked) {
        LOGE("%s: Assignment to a locked CameraMetadata!", __FUNCTION__);
        return *this;
    }
...
status_t CameraMetadata::unlock(const icamera_metadata_t *buffer) {
    if (!mLocked) {
        LOGE("%s: Can't unlock a non-locked CameraMetadata!", __FUNCTION__);
...
icamera_metadata_t* CameraMetadata::release() {
    if (mLocked) {
        LOGE("%s: CameraMetadata is locked", __FUNCTION__);
        return NULL;
    }
...
status_t CameraMetadata::updateImpl(uint32_t tag, const void *data,
    ...
    if (type == -1) {
        LOGE("%s: Tag %d not found", __FUNCTION__, tag);
        return BAD_VALUE;
    }
    ...
    if (res != OK) {
        LOGE("%s: Unable to update metadata entry %s.%s (%x): %s (%d)",
                __FUNCTION__, get_icamera_metadata_section_name(tag),
...
status_t CameraMetadata::resizeIfNeeded(size_t extraEntries, size_t extraData) {
    if (mBuffer == NULL) {
        ...
        if (mBuffer == NULL) {
            LOGE("%s: Can't allocate larger metadata buffer", __FUNCTION__);
            return NO_MEMORY;
        }
    } else {
...
void CameraMetadata::swap(CameraMetadata& other) {
    if (mLocked) {
        ...
        LOGE("%s: Other CameraMetadata is locked", __FUNCTION__);
        return;
    }

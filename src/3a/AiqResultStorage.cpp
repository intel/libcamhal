...
map<int, AiqResultStorage*> AiqResultStorage::sInstances;
...
ltm_result_t* AiqResultStorage::acquireLtmResult()
{
    ...
    mLtmResult[index].sequence = -1;

    return &mLtmResult[index];
}
...
void AiqResultStorage::updateLtmResult(long sequence)
{
    ...
    mLtmResult[mCurrentLtmIndex].sequence = sequence;
}
...
const ltm_result_t* AiqResultStorage::getLtmResult(long sequence)
{
    ...
    if (sequence == -1) {
        return &mLtmResult[mCurrentLtmIndex];
    }
    ...
    for (int i = 0; i < kLtmStorageSize; i++) {
        ...
        if (mLtmResult[tmpIdx].sequence >= 0 && sequence >= mLtmResult[tmpIdx].sequence) {
            LOG2("%s, find the ltm result (expect: %ld actual: %ld)",
                    __func__, sequence, mLtmResult[tmpIdx].sequence);
            return &mLtmResult[tmpIdx];
        }
    }
...
ltm_tuning_data* AiqResultStorage::acquireLtmTuningData()
{
    AutoWMutex rlock(mDataLock);

    int index = mCurrentLtmTuningIndex + 1;
    index %= kLtmTuningStorageSize;

    return &mLtmTuningData[index];
}

void AiqResultStorage::updateLtmTuningData()
{
    AutoWMutex wlock(mDataLock);

    mCurrentLtmTuningIndex++;
    mCurrentLtmTuningIndex %= kLtmTuningStorageSize;
}

const ltm_tuning_data* AiqResultStorage::getLtmTuningData()
{
    AutoRMutex rlock(mDataLock);

    if (mCurrentLtmTuningIndex == -1)
        return nullptr;

    return &mLtmTuningData[mCurrentLtmTuningIndex];
}
// LOCAL_TONEMAP_E
...
const DvsResult* AiqResultStorage::getDvsResult()
{
    AutoRMutex rlock(mDataLock);
    ...
    Check(mDvsResults[mCurrentDvsIndex]->mSequence == -1, nullptr, "invalid sequence id -1");
...
const AiqStatistics* AiqResultStorage::getAndLockAiqStatistics()
{
    ...
    Check(mAiqStatistics[mCurrentAiqStatsIndex].mSequence == -1,

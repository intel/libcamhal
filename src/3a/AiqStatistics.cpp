...
AiqStatistics &AiqStatistics::operator=(const AiqStatistics &other)
{
    ...
    saveHdrRgbsGridData(&other.mHdrRgbsGrid);
...
{
    Check(rgbsGrids == nullptr, BAD_VALUE, "Rgbs grid is null");

    for (int i = 0; i < exposureNum; i++) {
        int ret = copyRgbsGridData(rgbsGrids[i], &mRgbsGridArray[i]);
        Check(ret != OK, ret, "save Rgbs grid %d failed ret %d", i, ret);
    }
...
{
    LOG3A("%s", __func__);
    Check(src == nullptr || dst == nullptr, BAD_VALUE, "src or dst rgbs grid is null");
...
int AiqStatistics::saveHdrRgbsGridData(const ia_aiq_hdr_rgbs_grid *hdrRgbsGrid)
{
    LOG3A("%s", __func__);
    Check(hdrRgbsGrid == nullptr, BAD_VALUE, "hdr rgbs grid is null");

    // Release the old memory if size changes.
    if (GRID_SIZE_UNEQUAL(hdrRgbsGrid, &mHdrRgbsGrid)) {
        if (mHdrRgbsGrid.blocks_ptr) delete [] mHdrRgbsGrid.blocks_ptr;
        CLEAR(mHdrRgbsGrid);
    }

    if (hdrRgbsGrid->blocks_ptr == nullptr
        || hdrRgbsGrid->grid_width == 0 || hdrRgbsGrid->grid_height == 0) {
        return OK;
    }

    size_t gridSize = hdrRgbsGrid->grid_width * hdrRgbsGrid->grid_height;
    if (mHdrRgbsGrid.blocks_ptr == nullptr) {
        mHdrRgbsGrid.blocks_ptr = new hdr_rgbs_grid_block[gridSize];
    }

    mHdrRgbsGrid.grid_width = hdrRgbsGrid->grid_width;
    mHdrRgbsGrid.grid_height = hdrRgbsGrid->grid_height;
    MEMCPY_S(mHdrRgbsGrid.blocks_ptr, gridSize * sizeof(hdr_rgbs_grid_block),
             hdrRgbsGrid->blocks_ptr, gridSize * sizeof(hdr_rgbs_grid_block));

    LOG3A("%s, grid size=[%dx%d]", __func__, mHdrRgbsGrid.grid_width, mHdrRgbsGrid.grid_height);
    return OK;
}
...
{
    LOG3A("%s", __func__);
    Check(afGrid == nullptr, BAD_VALUE, "AF grid is null");
